...
def execution_creator_auth(users, execution_creator_username):
    """
    Raises NonRecoverableError when neither execution_creator_username is not in users parameter nor he does not have sys_admin role.
...
def set_breakpoint_state(node_ids=None,
                         ...
                         ctx=None):
    """
    Runs for`cloudify.nodes.breakpoint.Breakpoint` node type. Admin user is always implicitly the allowed.
    :param node_ids: list of IDs of `cloudify.nodes.breakpoint.Breakpoint`
    :param node_instance_ids: list of IDs of `cloudify.nodes.breakpoint.Breakpoint` node instance, node ID is used interchangeably
    :param all_breakpoints: override the _ode (instance) ID lists and open all breakpoints
    :param break_on_start: specifies if the breakpoint should stop on `cloudify.interfaces.lifecycle.start` lifecycle operation
    :param break_on_stop: specifies if the breakpoint should stop on `cloudify.interfaces.lifecycle.stop` lifecycle operation
    :param permanent: specifies that this setting of the breakpoints is permanent, if false it will apply only to the next execution
